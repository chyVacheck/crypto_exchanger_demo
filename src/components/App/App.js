...
function App() {
  ...
  async function getRate(currency) {
    try {
      const data = await bitstampApi.getTickerData(currency.toLowerCase() + 'usd');
      return data.last;
    } catch {
  ...
  function infoPopupClose(e) {
    if (e.target === e.currentTarget || e.target.id === 'infoPopupButton' || e.key === 'Escape') {
  ...
  return (
    <CurrentUserContext.Provider value={currentUser}>
      <div className="app">
        <Header loggedIn={isLoggedIn} onClick={handleMenuClick} onLogout={handleLogout} windowSize={windowSize} />
        <Routes>
          <Route
            path="/"
            element={
          ...
          <Route
            ...
            element={
              <ProtectedRoute loggedIn={!isLoggedIn}>
                <Login onLogin={handleLogin} infoPopupState={infoPopupState} onInfoPopupClose={infoPopupClose} />
              </ProtectedRoute>
            }
          />
          <Route
            ...
            element={
              <ProtectedRoute loggedIn={!isLoggedIn}>
                <Signup onSignup={handleSignup} infoPopupState={infoPopupState} onInfoPopupClose={infoPopupClose} />
              </ProtectedRoute>
            }
          />
          <Route
          ...
          />
          <Route path="*" element={<PageNotFound onReturn={() => navigate(-1)} />} />
        </Routes>
        <Footer />
        <PopupMenu loggedIn={isLoggedIn} isOpen={popupMenuState} onClose={closePopup} onLogout={handleLogout} />
      </div>
