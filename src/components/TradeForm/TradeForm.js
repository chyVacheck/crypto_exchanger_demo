...
function TradeForm(props) {
  ...
  const handleFieldChange = (e) => {
    ...
    setIsFormValid(validatedInputs.amount1.valid && validatedInputs.amount2.valid);
  };
  ...
  function calculateInputValue({ value, rate }) {
    if (!value) {
      ...
      const fractionLength = ('' + Math.trunc(calcAmount)).length > 9 ? 0 : 9 - ('' + Math.trunc(calcAmount)).length;
    ...
    <>
      {props.infoPopupState.isOpen &&
        (props.infoPopupState.isError ? (
          <InfoPopup message={alertText.transactionError} imgType={false} onClose={props.onInfoPopupClose} />
        ) : (
          <InfoPopup message={alertText.transactionCompleted} imgType={true} onClose={props.onInfoPopupClose} />
        ))}
      <div className="tradeForm">
        <p className="tradeForm__title">Trade SIMULATION</p>
        <Form buttonText="Confirm transaction" onSubmit={handleSubmit} isFormValid={currentUser.name && isFormValid}>
          <div className="form__inputs-box">
            <label className="form__inputs">
              ...
              <button className="form__droplist-button" id="cur1" type="button" onClick={props.openCurrenciesPopup}>
                {currencyPair.name1}&nbsp;&#9013;
              </button>
            </label>
            <div className="form__alerts">
              <span className="form__input-alert form__input-alert_orange" id="currency1-alert">
                {validatedInputs.amount1.alert}
              </span>
              <span className="form__input-alert form__input-alert_pink" id="rate1-alert">
                {`${currencyPairRates.rate1 ? `USD/${currencyPair.name1}=${currencyPairRates.rate1}` : ''}`}
              </span>
            </div>
          </div>

          <div className="form__swap-box">
            <button className="form__swap-button" type="button" onClick={swapValues}>
              &#8645;
            </button>
          </div>
          ...
          <div className="form__inputs-box">
            <div className="form__inputs">
              <input
                ...
                onChange={handleFieldChange}
              ></input>
              <button className="form__droplist-button" id="cur2" type="button" onClick={props.openCurrenciesPopup}>
                {currencyPair.name2}&nbsp;&#9013;
              </button>
            </div>
            <div className="form__alerts">
              ...
              <span className="form__input-alert form__input-alert_pink" id="rate1-alert">
                {`${currencyPairRates.rate1 ? `USD/${currencyPair.name2}=${currencyPairRates.rate2}` : ''}`}
              </span>
            </div>
          </div>
