    ...
    <>
      {props.infoPopupState.isOpen &&
        (props.infoPopupState.isError ? (
          <InfoPopup message={alertText.updateEror} imgType={false} onClose={props.onInfoPopupClose} />
        ) : (
          <InfoPopup message={alertText.profileUpdated} imgType={true} onClose={props.onInfoPopupClose} />
        ))}
      <div className="profile">
        <p className="profile__title">{`Hi, ${currentUser.name}!`}</p>
        <Form buttonText="Save changes" onSubmit={handleSubmit} isFormValid={isFormValid}>
          ...
          <span className={`form__input-alert ${validatedFields.name ? '' : 'form__input-alert_pink'}`} id="name-alert">
            {alertText.name}
          </span>
          <label className="form__inputs-box">
            E-mail
          ...
          <span
            className={`form__input-alert ${validatedFields.email ? '' : 'form__input-alert_pink'}`}
            id="email-alert"
          >
            {alertText.email}
          </span>
        ...
        <p className="form__bottom-text">
          ...
          <Link to="/logout" className="form__bottom-link link-effect" onClick={props.onLogout}>
            Log out
          </Link>
        </p>
      </div>
