  ...
  return (
    <section className="landing">
      <article className="landing__infoblock">
        ...
        <h2 className="landing__subtitle">It's NOT a real trading platform. Everything you see here is a mock-up.</h2>
        <ul className="landing__stats">
          {landingStats.map((item) => (
            <li className="landing__stats-content" key={item.name}>
              {item.value}
              <span className="landing__stats-content_light-text">{item.name}</span>
            </li>
          ))}
        </ul>
      </article>
      <article className="landing__currencies">
        <div className="landing__currency-title">
          ...
          <p className="landing__text">Please, register to create your demo account and demo wallet.</p>
          <Link to="/signup" className="landing__register-link link-effect">
            Register now
          </Link>
        </div>
