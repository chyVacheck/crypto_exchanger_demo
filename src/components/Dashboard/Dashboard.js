...
function Dashboard(props) {
  ...
  React.useEffect(() => {
    ...
    const ownFunds = currencyList.filter((item) => Object.keys(props.wallet.currencies).includes(item.symbol));
  ...
  return (
    <section className="dashboard">
      {props.infoPopupState.isError && (
        <InfoPopup message={alertText.ratesUpdateError} imgType={false} onClose={props.onInfoPopupClose} />
      )}
      <div className="dashboard__section">
        <UserData></UserData>
        <UserWallet walletData={walletData.ownFunds} isRetrievingData={isRetrievingData}></UserWallet>
      </div>
      <div className="dashboard__section">
        <UserStats totalFunds={walletData.ownFundsUsd} transactions={transactionsData.length}></UserStats>
        <UserDeals isRetrievingData={isRetrievingData} transactionsData={transactionsData}></UserDeals>
      </div>
    </section>
  );
}
