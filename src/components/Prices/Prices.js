...
function Prices(props) {
  ...
  return (
    <section className="prices">
      <p>
        Last updated:&nbsp;
        {JSON.stringify(priceData) === '{}' ? 'awaiting update ...' : updateDate}
      </p>
          ...
          <table className="prices__table">
            <thead>
              <tr className="prices__line">
                <th scope="col" className="prices__data">
                  Trading Pair
                </th>
                <th scope="col" className="prices__data">
                  Buy
              ...
              {Array.from(priceData).map((item) => {
                return (
                  <tr className="prices__line" key={item.pair}>
                    ...
                    {props.windowSize > 480 && <td className="prices__data">{item.last}</td>}
                    <td
                      className={`prices__data ${
                        Number(item.percent_change_24) < 0 ? `prices__data_red` : `prices__data_green`
                      }`}
                    >
                    ...
                    {props.windowSize > 480 && (
                      <td className="prices__data prices__data_right">
                        {Intl.NumberFormat('en-GB').format(Number(item.volume).toFixed(2))}
                      </td>
                    )}
                  </tr>
                );
